...
public class InferPostOrder {
  ...
  public static void main(String[] args) {
    ...
    root =
        new InferPostOrder()
            .build(
                new int[] {1, 2, 4, 7, 3, 5, 6, 8}, new int[] {4, 7, 2, 1, 5, 3, 8, 6}, 0, 7, 0, 7);
  ...
  public Node build(int[] pre, int[] mid, int preStart, int preEnd, int midStart, int midEnd) {
    ...
    if (partitionIndex > midStart) {
      Node left =
          build(
              ...
              mid,
              preStart + 1,
              preStart + partitionIndex - midStart,
              midStart,
              partitionIndex - 1);
      rootNode.left = left;
    }
    if (partitionIndex < midEnd) {
      Node right =
          build(pre, mid, preEnd - midEnd + partitionIndex + 1, preEnd, partitionIndex + 1, midEnd);
